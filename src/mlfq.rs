...
impl MLFQ {
    ...
    pub fn execute_process(&mut self, queue_index: usize) {
        if let Some(process) = self.queues[queue_index].get_mut(0) {
            println!("Executing process ID: {}", process.id); // Use the id field here
            let time_quantum = self.time_quanta[queue_index];
            let time_to_run = std::cmp::min(time_quantum, process.remaining_time);
            process.remaining_time -= time_to_run;
            process.total_executed_time += time_to_run;
            self.current_time += time_to_run;
    
            if process.remaining_time == 0 {
                println!("Process ID: {} completed.", process.id); // Process completed
                self.queues[queue_index].remove(0); 
            } else if queue_index < self.num_levels - 1 {
                let (current_queue, lower_queue) = self.queues.split_at_mut(queue_index + 1);
                lower_queue[0].push(current_queue[queue_index].remove(0));
            }
        }
    }
